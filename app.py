...
====================================
Every route is wrapped in try/except so a crash prints the REAL traceback
to Render logs instead of silently serving the 500 page.
"""
...
# ── FIX 1: SECRET_KEY must exist and be stable across all Gunicorn workers ───
# If SECRET_KEY is missing or different between workers, flash() and session
# both silently fail → redirect appears to crash after insert.
SECRET_KEY = os.environ.get("SECRET_KEY", "")
if not SECRET_KEY:
    # Fallback for local dev only — will NOT work reliably on Render
    SECRET_KEY = "local-only-dev-key-do-not-use-in-production"
    logging.warning(
        "⚠️  SECRET_KEY is not set! Sessions and flash messages will "
        "NOT work correctly across Gunicorn workers. "
        "Add SECRET_KEY to your Render environment variables."
    )
app.secret_key = SECRET_KEY

# ── FIX 2: SESSION_COOKIE_SECURE must only be True when HTTPS is confirmed ───
# If HTTPS=true but your domain is still HTTP → browser blocks the cookie →
# flash() message is lost → next page looks like a crash.
HTTPS_ENABLED = os.environ.get("HTTPS", "false").lower() == "true"
...
# ── Logging: show full tracebacks in Render log viewer ───────────────────────
logging.basicConfig(
    level=logging.INFO,
...
def student_register():
    # ── FIX 3: wrap entire route in try/except so the REAL error appears
    # in Render logs instead of being swallowed by the 500 handler ────────────
    try:
        if request.method == "POST":
            ...
            # ── FIX 4: flash BEFORE redirect; verify route name is exact ─────
            # "student_login" must match the function name below exactly.
            flash("Registration successful! Please login.", "success")
            logger.info("New student registered: %s", email)
            return redirect(url_for("student_login"))   # ← function name must match

        # GET
        return render_template("student_register.html")   # ← filename must match exactly (Linux is case-sensitive)

    except Exception:
        # Print the FULL traceback to Render logs
        logger.error("ERROR in student_register:\n%s", traceback.format_exc())
...
def student_login():                          # ← this name must match url_for("student_login")
    try:
        if request.method == "POST":
            email    = request.form.get("email", "").strip().lower()
            password = request.form.get("password", "")
...
def recruiter_register():
    try:
        if request.method == "POST":
            ...
            flash("Registration successful! Please login.", "success")
            logger.info("New recruiter registered: %s", email)
            return redirect(url_for("recruiter_login"))  # ← must match function name

        return render_template("recruiter_register.html")

    except Exception:
        logger.error("ERROR in recruiter_register:\n%s", traceback.format_exc())
...
def recruiter_login():                       # ← must match url_for("recruiter_login")
    try:
        if request.method == "POST":
            email     = request.form.get("email", "").strip().lower()
            password  = request.form.get("password", "")
...
# ══════════════════════════════════════════════════════════════════════════════
#  ERROR HANDLERS
#  These only trigger when a route does NOT have its own try/except.
#  With the try/except in every route above, these now only catch truly
#  unexpected framework-level errors (e.g. Jinja2 template not found).
# ══════════════════════════════════════════════════════════════════════════════
...
def server_error(e):
    # e contains the original exception — log it fully
